...
#define M_X_MINd -2566
#define M_Y_MINd -2723
#define M_Z_MINd -2993
#define M_X_MAXd 5121
#define M_Y_MAXd 3067
#define M_Z_MAXd 3483
...
//Dependiendo del estado del switch vamos a un estado u otro: 
  if ((lectura_RB=='1' )&& (digitalRead(pin_interruptor) == HIGH)){estado=14;sonidostart();parpadeoLed(0);}
    ...
    case 0: 
     ...
     if (digitalRead(pin_pulsador1) == 1 && pulsador_apagar==1 ){    //Apagado del sistema:
      delay(5);
      timer_restart=millis();
      pulsador_apagar=0;
     }
     else if (digitalRead(pin_pulsador1)==0){timer_restart=millis(); pulsador_apagar=1; }

    if ((millis()-timer_restart)>=2000 && digitalRead(pin_pulsador1)==1){//si mantenemos el pulsador 2 segundos apagamos sistema
...
//deben estar los zapatos insertados para calibrar las imus
   ...
   if(FCH_1==LOW && FCH_2==LOW){
    ...
    lcd.print("   HORIZONTAL   ");
     ...
     if (digitalRead(pin_pulsador1) == 1) {estado = 10; delay(10);primeraVez=1;};
   }
...
//          sonidoOK();     
    ...
    num_vueltas=0;
      ...
      if (digitalRead(pin_pulsador1) == 1)//pasamos a realizar otra calibración al pulsar
      {
