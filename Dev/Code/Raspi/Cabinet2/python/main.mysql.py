...
try:
    ...
    while(connected==False):
        try:
            mega = serial.Serial("/dev/ttyUSB0",115200,timeout=0.2,dsrdtr=True )
    ...
    while(1):
        ...
        if (comando[:4] == "IZUM"):  # Comando de inicizalizacion de la zum
            ...
            # Programamos la ZUM
            try:
                subprocess.check_call("avrdude -patmega328p -carduino -P/dev/ttyUSB1 -b 115200 -D -Uflash:w:/home/pi/zowi/python/zowi_offset_i2c.cpp.hex:i",shell=True)
            ...
            # Si no se produce ningún error nos conectamos a la ZUM
                ...
                try:
                    zum = serial.Serial("/dev/ttyUSB1",115200,timeout=0.2)
                except serial.SerialException:
	    ...
	    ####################################################
         ...
         #######################################################
            ...
            zum.close()
            try:
                subprocess.check_call("avrdude -patmega328p -carduino -P/dev/ttyUSB1 -b 115200 -D -Uflash:w:/home/pi/zowi/python/ZOWI_BASE_v0.hex:i",shell=True)
            except subprocess.CalledProcessError:
...
###
    ...
    print("Mega desconectada de forma inorreta")
    try:
        db_localhost.close()
    except:
        print("[X] Error al cerrar base de datos local")
